...
_PIM_OPS = {OpCode.PIM_MAC, OpCode.PIM_EW_OP, OpCode.PIM_REDUCE}
_READ_OPS = {OpCode.SRAM_RD, OpCode.DMA_LOAD, OpCode.DMA_PREFETCH}
_WRITE_OPS = {OpCode.SRAM_WR, OpCode.DMA_STORE}
    ...
    def can_issue(self, op: OpCode, banks: list, nbytes: int = 0) -> bool:
        ...
        for b in banks:
            ...
            if self.bank_busy_until[b] > self.current_cycle:
                ...
                # Requesting a PIM op on a busy bank
                if op in _PIM_OPS:
                    if port.pim_exclusive_with_read or port.pim_exclusive_with_write:
                        self.bank_conflict_count += 1
                        return False
                # Requesting a read/write on a bank busy with PIM
                if current_op in _PIM_OPS:
                    if op in _READ_OPS and port.pim_exclusive_with_read:
                        self.bank_conflict_count += 1
                        return False
                    if op in _WRITE_OPS and port.pim_exclusive_with_write:
                        self.bank_conflict_count += 1
                        return False
